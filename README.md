...
Дело в том что если сразу во время сканирования выводить результаты, то потоки будут обганять друг друга, и мало того что будут одни утечки памяти,
так ещё и за счёт гонки, лог будет очень кривой и странный.

- Плюс схемы, в том что сканирование и все операции, выполняються без каких либо внешних и внутренних задержок, так ещё и в куче потоков. За счёт этого,
...
- `TXT:` Чисто лог, не больше чем лог. Не удобно вообщем, но в ![nmap](https://nmap.org/download.html) всем зашло.
