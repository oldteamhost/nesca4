...
class checking_finds{
    ...
    std::vector<std::string> basic_auth_paths = {"/view/getuid.cgi", "/cgi-bin/guest/Video.cgi?", "/admin/index.shtml?", 
									...
									"/control/userimage.html", "/view/viewer_index.shtml", "/view/viewer_index.shtml", 
                                    "/mjpg/video.mjpg", "/axis-cgi/com/ptz.cgi?","/view/viewer_index.shtml?", "check_user.cgi",
									"/axis-cgi/com/ptz.cgi?query=position", "CgiStart?page=", "view/indexFrame.shtml"};

    std::string
	check_basic_auth(std::string ip);
};
