  ...
  u8 *ipopt, int ipoptlen, u16 sport, u16 dport, const char *data, u16 datalen, int fragscan)
{
  ...
  int res = -1;
  u8 *packet = build_udp_pkt(saddr, daddr, ttl, ipid, IP_TOS_DEFAULT, false, ipopt, ipoptlen, sport, dport, data, datalen, &packetlen);
  ...
  dst_in->sin_addr.s_addr = daddr; /* add .s_addr */
