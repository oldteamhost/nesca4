...
void init_uri(struct uri *u, const char *scheme, const char *host, u16 port,
              const char *path);
void print_uri(struct uri *u);
void free_uri(struct uri *u);
void update_uri(struct uri *u, const char *scheme, const char *host, u16 port,
                const char *path);
void init_http_header(struct _http_header *h, const char *field,
                      const char *value);
...
void add_http_basicauth(struct http_request *h, const char *user,
                        const char *pass);
u8 *build_http_request_pkt(struct http_request *request, const char *data,
                           ssize_t datalen, ssize_t *plen);
u8 *remove_html(u8* packet);
struct http_response read_http_request_pkt(u8 *response);
void get_http_title(const char *http_header, char *title_buffer,
                    size_t buffer_size);
char *parse_parent_location(const char *html);
char *parse_url_from_js(const char *html);
char *parse_http_equiv(const char *html);
void find_http_redirect(struct _http_header *h, u8 *packet, char *res,
                        ssize_t reslen);
...
int sendfast_http_request(const char *ip, u16 port, const char *path,
                          int timeout_ms, struct http_response *r, u8 *packet,
                          ssize_t len);
int sendfast_http_custom(const char *ip, u16 port, int timeout_ms,
                         struct http_request *r, struct http_response *res,
                         u8 *packet, ssize_t len);
bool auth_http_basic(int fd, const char *ip, const char *path, const char *user,
                     const char *pass);
